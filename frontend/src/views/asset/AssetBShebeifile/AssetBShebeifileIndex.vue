            ...
            >
                ...
                <div v-else>
                    <a-date-picker
                            ...
                            @change="(e,f) => handleChange(e,f,record,'scDate')"
                    />
                </div>
            </template>
      <template
...
<script>
   ...
   if(         element.baseId!= ''       ||    element.scDate!= ''       ||    element.type!= ''       ||    element.fileId!= ''       ||    element.fileUrl!= ''          ||    element.note!= ''                     ){
                ...
                if (dataAdd.length === 0) {
                    ...
                    this.loading = true
                    this.$post('assetBShebeifile/addNew', {
                        jsonStr: jsonStr,
                        state: 0
                    }).then(() => {
                        // this.reset()
   ...
   if(         element.baseId!= ''       ||    element.scDate!= ''       ||    element.type!= ''       ||    element.fileId!= ''       ||    element.fileUrl!= ''          ||    element.note!= ''                     ){
                        ...
                        if (dataAdd.length === 0) {
                            ...
                            let jsonStr = JSON.stringify(dataAdd)
                            that.loading = true
                            that.$post('assetBShebeifile/addNew', {
                                jsonStr: jsonStr,
                                state: 1
                            }).then(() => {
                                //this.reset()
            ...
            fetch () {
                this.$get('assetBShebeifile/custom', {
                }).then((r) => {
                    let data = r.data
        ...
        computed: {
            ...
            return [                        {
                        ...
                        },
                        {
                            ...
                            dataIndex: 'scDate',
                            width: 130,
                            scopedSlots: { customRender: 'scDate' }
                        },
                        {
                                title: '文件类型',
                            dataIndex: 'type',
