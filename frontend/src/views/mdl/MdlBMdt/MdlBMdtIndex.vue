  ...
  <a-card title="">
    ...
    >
      <template slot="teamName" slot-scope="text, record">
        ...
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'teamName')"
            :value="record.teamName"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="deptHead" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'deptHead')"
            :value="record.deptHead"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="userAccountNameLeader" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="
              (e) =>
                inputChange(e.target.value, record, 'userAccountNameLeader')
            "
            :value="record.userAccountNameLeader"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="userAccountLeader" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="
              (e) => inputChange(e.target.value, record, 'userAccountLeader')
            "
            :value="record.userAccountLeader"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="userAccountNameAssist" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="
              (e) =>
                inputChange(e.target.value, record, 'userAccountNameAssist')
            "
            :value="record.userAccountNameAssist"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="userAccountAssist" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="
              (e) => inputChange(e.target.value, record, 'userAccountAssist')
            "
            :value="record.userAccountAssist"
          >
          </a-textarea>
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      dataSource.forEach((element) => {
        if (
          element.teamName != "" ||
          element.deptHead != "" ||
          element.userAccountNameLeader != "" ||
          element.userAccountLeader != "" ||
          element.userAccountNameAssist != "" ||
          element.userAccountAssist != ""
        ) {
    ...
    fetch() {
      this.$get("mdlBMdt/custom", {}).then((r) => {
        let data = r.data;
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          dataIndex: "deptHead",
          width: 130,
          scopedSlots: { customRender: "deptHead" },
        },
        {
          title: "团队负责人",
          dataIndex: "userAccountNameLeader",
          width: 130,
          scopedSlots: { customRender: "userAccountNameLeader" },
        },
        {
          title: "团队负责人发薪号",
          dataIndex: "userAccountLeader",
          width: 130,
          scopedSlots: { customRender: "userAccountLeader" },
        },
        {
          title: "团队秘书",
          dataIndex: "userAccountNameAssist",
          width: 130,
          scopedSlots: { customRender: "userAccountNameAssist" },
        },
        {
          title: "团队秘书发薪号",
          dataIndex: "userAccountAssist",
          width: 130,
          scopedSlots: { customRender: "userAccountAssist" },
        },
        {
          title: "状态",
          dataIndex: "state",
