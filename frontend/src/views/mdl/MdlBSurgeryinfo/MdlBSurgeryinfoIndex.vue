<template>
    <a-card title="">
      ...
      <template
          ...
          <div v-else>
              <a-textarea
                      @blur="e => inputChange(e.target.value,record,'jb')"
                      :value="record.jb"
              >
              </a-textarea>
          </div>
...
<script>
   ...
   if(         element.deptName!= ''       ||    element.userAccountName!= ''       ||    element.userAccount!= ''       ||    element.zhicheng!= ''       ||    element.sexName!= ''       ||    element.birthday!= ''       ||    element.jb!= ''       ||    element.validDate!= ''       ||    element.note!= ''                     ){
                ...
                if (dataAdd.length === 0) {
                    ...
                    this.loading = true
                    this.$post('mdlBSurgeryinfo/addNew', {
                        jsonStr: jsonStr,
                        state: 0
                    }).then(() => {
                        // this.reset()
   ...
   if(         element.deptName!= ''       ||    element.userAccountName!= ''       ||    element.userAccount!= ''       ||    element.zhicheng!= ''       ||    element.sexName!= ''       ||    element.birthday!= ''       ||    element.jb!= ''       ||    element.validDate!= ''       ||    element.note!= ''                     ){
                        ...
                        if (dataAdd.length === 0) {
                            ...
                            let jsonStr = JSON.stringify(dataAdd)
                            that.loading = true
                            that.$post('mdlBSurgeryinfo/addNew', {
                                jsonStr: jsonStr,
                                state: 1
                            }).then(() => {
                                //this.reset()
            ...
            fetch () {
                this.$get('mdlBSurgeryinfo/custom', {
                }).then((r) => {
                    let data = r.data
