<template>
    <a-card title="">
        ...
        >
            ...
            <div v-else>
                <a-input-number
                        @blur="e => inputChange(e.target.value,record,'sugeryId')"
                        :value="record.sugeryId"
                        :precision="0"
                >
                </a-input-number>
            </div>
      ...
      <template
          ...
          <div v-else>
              <a-textarea
                      @blur="e => inputChange(e.target.value,record,'code')"
                      :value="record.code"
              >
              </a-textarea>
          </div>
      ...
      <template
          ...
          <div v-else>
              <a-textarea
                      @blur="e => inputChange(e.target.value,record,'level')"
                      :value="record.level"
              >
              </a-textarea>
          </div>
...
<script>
   ...
   if(         element.baseId!= ''       ||    element.sugeryId!= ''       ||    element.deptNew!= ''       ||    element.code!= ''       ||    element.name!= ''       ||    element.level!= ''       ||    element.lb!= ''                     ){
                ...
                if (dataAdd.length === 0) {
                    ...
                    this.loading = true
                    this.$post('mdlBSurgeryinfoD/addNew', {
                        jsonStr: jsonStr,
                        state: 0
                    }).then(() => {
                        // this.reset()
   ...
   if(         element.baseId!= ''       ||    element.sugeryId!= ''       ||    element.deptNew!= ''       ||    element.code!= ''       ||    element.name!= ''       ||    element.level!= ''       ||    element.lb!= ''                     ){
                        ...
                        if (dataAdd.length === 0) {
                            ...
                            let jsonStr = JSON.stringify(dataAdd)
                            that.loading = true
                            that.$post('mdlBSurgeryinfoD/addNew', {
                                jsonStr: jsonStr,
                                state: 1
                            }).then(() => {
                                //this.reset()
            ...
            fetch () {
                this.$get('mdlBSurgeryinfoD/custom', {
                }).then((r) => {
                    let data = r.data
        ...
        computed: {
            ...
            return [                        {
                        ...
                        },
                        {
                            ...
                            dataIndex: 'sugeryId',
                            width: 130,
                            scopedSlots: { customRender: 'sugeryId' }
                        },
                        {
                                title: '科室',
                            dataIndex: 'deptNew',
