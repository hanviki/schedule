<template>
    <a-card title="操作分级目录及资质授权">
      ...
      <template
          ...
          <div v-else>
              <a-textarea
                      @blur="e => inputChange(e.target.value,record,'docto')"
                      :value="record.docto"
              >
              </a-textarea>
          </div>
...
<script>
   ...
   if(         element.deptNew!= ''       ||    element.code!= ''       ||    element.name!= ''       ||    element.level!= ''       ||    element.docto!= ''                     ){
                ...
                if (dataAdd.length === 0) {
                    ...
                    this.loading = true
                    this.$post('mdlDOperationDoctor/addNew', {
                        jsonStr: jsonStr,
                        state: 0
                    }).then(() => {
                        // this.reset()
   ...
   if(         element.deptNew!= ''       ||    element.code!= ''       ||    element.name!= ''       ||    element.level!= ''       ||    element.docto!= ''                     ){
                        ...
                        if (dataAdd.length === 0) {
                            ...
                            let jsonStr = JSON.stringify(dataAdd)
                            that.loading = true
                            that.$post('mdlDOperationDoctor/addNew', {
                                jsonStr: jsonStr,
                                state: 1
                            }).then(() => {
                                //this.reset()
            ...
            fetch () {
                this.$get('mdlDOperationDoctor/custom', {
                }).then((r) => {
                    let data = r.data
        ...
        computed: {
            ...
            return [                        {
                        ...
                        },
                        {
                            ...
                            dataIndex: 'docto',
                            width: 130,
                            scopedSlots: { customRender: 'docto' }
                        },
                {
                    title: '状态',
                    dataIndex: 'state',
