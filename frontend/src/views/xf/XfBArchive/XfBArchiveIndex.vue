<template>
  <a-card title="">
    ...
    >
      ...
      <template slot="fileCode" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'fileCode')"
            :value="record.fileCode"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="fileDate" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
        ...
        <div v-else>
          <a-date-picker
            ...
            "
            @change="(e, f) => handleChange(e, f, record, 'fileDate')"
          />
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      this.$get("xfBArchive/custom", {}).then((r) => {
        let data = r.data;
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          dataIndex: "fileDate",
          width: 130,
          scopedSlots: { customRender: "fileDate" },
        },
        {
          title: "文件类型",
          dataIndex: "fileType",
