...
<script>
import XfBAssetAdd from "./XfBAssetAdd";
import XfBAssetEdit from "./XfBAssetEdit";
...
export default {
  name: "XfBAsset",
  components: { XfBAssetAdd, XfBAssetEdit },
  data() {
    return {
      advanced: false,
      dataSource: [],
  ...
  methods: {
    ...
    edit(record) {
      this.$refs.xfBAssetEdit.setFormValues(record);
    ...
    batchDelete() {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk() {
          let xfBAssetIds = that.selectedRowKeys.join(",");
          that.$delete("xfBAsset/" + xfBAssetIds).then(() => {
