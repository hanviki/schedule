            ...
            >
                ...
                <div v-else>
                    <a-date-picker
                            ...
                            @change="(e,f) => handleChange(e,f,record,'meetDate')"
                    />
                </div>
            </template>
      <template
          ...
          <div v-else>
              <a-textarea
                      @blur="e => inputChange(e.target.value,record,'person')"
                      :value="record.person"
              >
              </a-textarea>
          </div>
...
<script>
   ...
   if(         element.name!= ''       ||    element.address!= ''       ||    element.meetDate!= ''       ||    element.person!= ''       ||    element.content!= ''       ||    element.remark!= ''       ||    element.fileId!= ''       ||    element.fileUrl!= ''                     ){
                ...
                if (dataAdd.length === 0) {
                    ...
                    this.loading = true
                    this.$post('xfBConference/addNew', {
                        jsonStr: jsonStr,
                        state: 0
                    }).then(() => {
                        // this.reset()
   ...
   if(         element.name!= ''       ||    element.address!= ''       ||    element.meetDate!= ''       ||    element.person!= ''       ||    element.content!= ''       ||    element.remark!= ''       ||    element.fileId!= ''       ||    element.fileUrl!= ''                     ){
                        ...
                        if (dataAdd.length === 0) {
                            ...
                            let jsonStr = JSON.stringify(dataAdd)
                            that.loading = true
                            that.$post('xfBConference/addNew', {
                                jsonStr: jsonStr,
                                state: 1
                            }).then(() => {
                                //this.reset()
        ...
        computed: {
            ...
            return [                        {
                        ...
                        },
                        {
                            ...
                            dataIndex: 'meetDate',
                            width: 130,
                            scopedSlots: { customRender: 'meetDate' }
                        },
                        {
                                title: '参会人员',
                            dataIndex: 'person',
