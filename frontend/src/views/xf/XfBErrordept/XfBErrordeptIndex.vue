  ...
  <a-card title="">
    ...
    >
      ...
      <template slot="result" slot-scope="text, record">
        ...
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'result')"
            :value="record.result"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="feedbackDate" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
        ...
        <div v-else>
          <a-date-picker
            ...
            "
            @change="(e, f) => handleChange(e, f, record, 'feedbackDate')"
          />
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      this.$get("xfBErrordept/custom", {}).then((r) => {
        let data = r.data;
