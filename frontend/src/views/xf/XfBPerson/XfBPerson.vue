...
<script>
import XfBPersonAdd from "./XfBPersonAdd";
import XfBPersonEdit from "./XfBPersonEdit";
...
export default {
  name: "XfBPerson",
  components: { XfBPersonAdd, XfBPersonEdit },
  data() {
    return {
      advanced: false,
      dataSource: [],
  ...
  methods: {
    ...
    edit(record) {
      this.$refs.xfBPersonEdit.setFormValues(record);
    ...
    batchDelete() {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk() {
          let xfBPersonIds = that.selectedRowKeys.join(",");
          that.$delete("xfBPerson/" + xfBPersonIds).then(() => {
