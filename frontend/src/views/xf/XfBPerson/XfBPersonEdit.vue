...
export default {
  ...
  methods: {
    ...
    onClose() {
     ...
     fetchFile() {
      this.$get("xfBArchive", { pageNum:1 ,pageSize: 10000, fileType: '人才项目' }).then((res) => {
        this.fileData = [];
    ...
    userChange(value, option) {
      console.info(option);
      this.xfBPerson["userAccountName"] = option.key.userAccountName;
      this.xfBPerson["userAccount"] = option.key.userAccount;
      this.xfBPerson["deptNew"] = option.key.deptNew;
      this.xfBPerson["zhicheng"] = option.key.zhicheng;
    },
    fileChange(value, option) {
      this.xfBPerson["archiveName"] = option.key.fileName;
      this.xfBPerson["archiveCode"] = option.key.fileCode;
      this.xfBPerson["fileId"] = option.key.fileId;
      this.xfBPerson["fileUrl"] = option.key.fileUrl;
    },
    setFormValues({ ...xfBPerson }) {
      let fields = [
        "userAccountName",
        "userAccount",
        "deptNew",
      ...
      Object.keys(xfBPerson).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key);
          let obj = {};
          if (fieldDates.indexOf(key) !== -1) {
            if (xfBPerson[key] !== null && xfBPerson[key] !== "") {
              obj[key] = moment(xfBPerson[key]);
             ...
             if (key == "archiveId") {
              if( xfBPerson[key]==null){
                obj[key] = "";
              }
              else {
              obj[key] = xfBPerson[key].toString();
              }

            } else {
              obj[key] = xfBPerson[key];
            }
            this.form.setFieldsValue(obj);
          }
        }
      });
      this.xfBPerson.id = xfBPerson.id;
      this.fetchUser(xfBPerson.userAccount)
    },
    setFields() {
      ...
      if (typeof values !== "undefined") {
        Object.keys(values).forEach((_key) => {
          this.xfBPerson[_key] = values[_key];
        });
      }
    },
    handleSubmit() {
