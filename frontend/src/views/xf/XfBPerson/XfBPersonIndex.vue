<template>
  <a-card title="">
    ...
    >
      ...
      <template slot="exiamResult" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'exiamResult')"
            :value="record.exiamResult"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="archiveId" slot-scope="text, record">
        <div v-if="record.state == 3">
          {{ text }}
        </div>
        <div v-else>
          <a-input-number
            @blur="(e) => inputChange(e.target.value, record, 'archiveId')"
      ...
      <template slot="archiveName" slot-scope="text, record">
        ...
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'archiveName')"
            :value="record.archiveName"
          >
          </a-textarea>
        </div>
      </template>
      <template slot="archiveCode" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
          {{ text }}
        </div>
        <div v-else>
          <a-textarea
            @blur="(e) => inputChange(e.target.value, record, 'archiveCode')"
            :value="record.archiveCode"
          >
          </a-textarea>
        </div>
      </template>
      ...
      <template slot="powerDate" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
        ...
        <div v-else>
          <a-date-picker
            ...
            "
            @change="(e, f) => handleChange(e, f, record, 'powerDate')"
          />
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      this.$get("xfBPerson/custom", {}).then((r) => {
        let data = r.data;
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          width: 130,
          scopedSlots: { customRender: "exiamResult" },
        },
        {
          title: "授权文件",
          dataIndex: "archiveId",
        ...
        },
        {
          ...
          width: 130,
          scopedSlots: { customRender: "archiveName" },
        },
        {
          title: "授权文件编码",
        ...
        },
        {
          ...
          dataIndex: "powerDate",
          width: 130,
          scopedSlots: { customRender: "powerDate" },
        },
        {
          title: "备注",
          dataIndex: "remark",
