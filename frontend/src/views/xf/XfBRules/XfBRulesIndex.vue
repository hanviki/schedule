<template>
  <a-card title="">
    ...
    >
      ...
      <template slot="reordDate" slot-scope="text, record">
        <div v-if="record.state == 3 || record.state == 1">
        ...
        <div v-else>
          <a-date-picker
            ...
            "
            @change="(e, f) => handleChange(e, f, record, 'reordDate')"
          />
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      this.$get("xfBRules/custom", {}).then((r) => {
        let data = r.data;
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          dataIndex: "reordDate",
          width: 130,
          scopedSlots: { customRender: "reordDate" },
        },
        {
          title: "附件ID",
          dataIndex: "fileId",
